...
RULES = (
    ...
    Rule(
        "assigned-secret",
        re.compile(
            r"(?i)\b(?:password|passwd|api[_-]?key|client[_-]?secret|access[_-]?token|refresh[_-]?token)\b"
            r"\s*[:=]\s*['\"]([^'\"\s]{8,})['\"]"
        ),
    ),
)
    ...
    for line_number, line in enumerate(text.splitlines(), start=1):
        ...
        for rule in RULES:
            for match in rule.pattern.finditer(line):
                ...
                if any(marker in lowered or marker in matched for marker in PLACEHOLDER_MARKERS):
                    continue
...
def main() -> int:
    ...
    if findings:
        ...
        return 1
    print(f"Sensitive-data scan passed for {len(iter_paths(sys.argv[1:]))} file(s).")
    return 0
