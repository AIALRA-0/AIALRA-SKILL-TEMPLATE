...
顶层契约使用以下文档注释版表示。代码块采用 `jsonc` 语法。`//` 注释仅用于解释字段；实际 `workflow.yaml` 必须删除全部注释，并保持可被标准 JSON 解析器读取。

```jsonc
{
  // 工作流 IR 协议版本。当前 Runtime 仅接受 1。
  // 发生不兼容的协议变更时必须升级该版本。
  "ir_version": 1,

  // Skill 的稳定标识符。
  // 必须与 .agents/skills/ 下的 Skill 目录名完全一致。
  "skill_name": "example-skill",

  // 领域工作流配置状态。
  // false：安全草稿，Runner 拒绝执行。
  // true：领域节点、Schema、测试和核心锁已经完成。
  "configured": true,

  // 每次执行进入的第一个节点 ID。
  // 该 ID 必须存在于 nodes 数组中。
  "entry_node": "normalize-input",

  // 单次执行的全局资源边界。
  "limits": {
    // 单次运行允许成功完成的最大节点数量，取值范围为 1–64。
    "max_nodes": 16,

    // 从 started_at 开始计算的最大总时长，单位为秒，取值范围为 1–86400。
    "total_timeout_seconds": 1800
  },

  // 受控学习的压缩配置。
  "learning": {
    // 活跃账本累计到该事件数量时触发压缩，取值范围为 4–1000。
    "compact_every": 32,

    // 注入 Runner 指令的活跃建议规则上限。
    // 取值范围为 1–500，并且不得超过 compact_every 的一半。
    "active_rule_limit": 16
  },

  // 工作流节点数组。实际文件必须至少包含一个完整节点。
  // 此处使用空数组表示顶层契约，不表示可执行配置。
  "nodes": [],

  // 最终输出 Schema 相对于 Skill 根目录的安全相对路径。
  // Runner 在完成前使用该 Schema 验证 final_output。
  "final_output_schema": "schemas/final.schema.json",

  // 最终确定性 validator 的 argv 数组。
  // null 表示最终输出仅执行 final_output_schema 验证。
