...
```jsonc
{
  "ir_version": 1, // 工作流 IR 协议版本；当前仅接受 1，不兼容变更必须升级版本。
  ...
  "nodes": [], // 工作流节点数组；实际文件至少包含一个完整节点，此处空数组仅表示顶层契约。
  "final_output_schema": "schemas/final.schema.json", // 最终输出 Schema 的安全相对路径。
  "final_validator": null // 最终 validator 的 argv 数组；null 表示仅执行最终 Schema 验证。
}
```
