...
用户请求
   ↓
Agent 生成符合入口 Schema 的 JSON
   ↓
Runner 创建状态并进入入口节点
   ↓
Runner 执行 script，或返回一个外部动作指令
   ↓
执行器返回符合节点输出 Schema 的 JSON
   ↓
Runner 执行 Schema 和 validator 检查
   ↓
成功：进入 on_success 节点
失败：执行有界重试、fallback、等待用户或停止
   ↓
最终输出通过 final Schema 和 final validator
   ↓
状态进入 completed
   ↓
学习系统记录一条脱敏事件
