...
```mermaid
flowchart TD
    A["用户请求"] --> B["Agent 构造入口 JSON"]

    subgraph START["阶段 1：启动与入口校验"]
        B --> C["runner.py start"]
        C --> D["定位仓库根目录与唯一 Skill"]
        D --> E["验证 workflow 结构、configured 状态与核心锁"]
        E --> F{"启动检查通过"}
        F -- "否" --> X0["返回 status=error<br/>拒绝创建可执行状态"]
        F -- "是" --> G["读取 entry_node 的输入 Schema"]
        G --> H{"入口 JSON 合规"}
        H -- "否" --> X1["返回 status=error<br/>报告输入错误"]
        H -- "是" --> I["创建 state_id<br/>status=running<br/>current_node=entry_node"]
    end

    subgraph DISPATCH["阶段 2：节点调度"]
        I --> J["runner.py advance"]
        J --> K{"max_nodes 与 total_timeout 检查通过"}
        K -- "否" --> X2["返回 status=error<br/>保留状态用于审计"]
        K -- "是" --> L{"当前节点需要确认且尚未批准"}

        L -- "是" --> WC["status=waiting-confirmation"]
        WC --> UC{"用户真实确认"}
        UC -- "未确认" --> WC
        UC -- "已确认" --> AP["runner.py approve<br/>status=running"]
        AP --> J

        L -- "否" --> M{"executor=script"}
        M -- "是" --> S1["写入节点 input.json"]
        S1 --> S2["按 argv 执行脚本<br/>shell=false<br/>应用节点 timeout"]
        S2 --> S3{"退出码为 0 且 output.json 存在"}
        S3 -- "否" --> FH["handle_failure<br/>kind=retryable"]
        S3 -- "是" --> V1["读取节点输出 JSON"]

        M -- "否" --> EX["返回外部执行指令<br/>executor、action、input、Schema<br/>副作用、timeout、停止条件、建议规则"]
        EX --> WE["status=waiting-external"]
        WE --> EO{"外部执行结果"}
        EO -- "产生输出" --> SUB["runner.py submit"]
        SUB --> ET{"在节点 timeout 内提交"}
        ET -- "否" --> FH
        ET -- "是" --> V1
        EO -- "报告失败" --> RF["runner.py fail<br/>提交失败类型和消息"]
        RF --> FH
    end

    subgraph VALIDATE["阶段 3：结果验证与状态转换"]
        V1 --> V2{"节点输出 Schema 通过"}
        V2 -- "否" --> FH
        V2 -- "是" --> V3["执行节点 validator"]
        V3 --> V4{"validator 通过"}
        V4 -- "否" --> FH
        V4 -- "是" --> R1["保存 node_results<br/>更新 current_input<br/>steps_executed 加 1"]
        R1 --> R2{"on_success=__complete__"}
        R2 -- "否" --> R3["current_node=on_success<br/>status=running"]
        R3 --> J
        R2 -- "是" --> FV1["执行 final output Schema"]
        FV1 --> FV2["执行 final validator"]
        FV2 --> FV3{"最终验证通过"}
        FV3 -- "否" --> FH
        FV3 -- "是" --> DONE["status=completed<br/>保存 final_output 与 completed_at"]
    end

    subgraph FAILURE["阶段 4：失败、回退与用户暂停"]
        FH --> FR{"retryable 且 retries 小于 max_retries"}
        FR -- "是" --> FR1["retries 加 1<br/>status=running"]
        FR1 --> J
        FR -- "否" --> FF{"失败类型允许回退且已声明 fallback"}
        FF -- "是" --> FF1["current_node=fallback<br/>status=running"]
        FF1 --> J
        FF -- "否" --> FU{"kind=user-required"}
        FU -- "是" --> WU["status=waiting-user"]
        WU --> U1["用户完成必要操作"]
        U1 --> U2["runner.py resume<br/>status=running"]
        U2 --> J
        FU -- "否" --> FAILED["status=failed<br/>保存 failed_at 与 last_error"]
    end

    subgraph LEARNING["阶段 5：完成记录与受控学习"]
        DONE --> LR["runner.py 返回 final_output<br/>learning_required=true"]
        LR --> L1["learn.py 校验核心锁、state_id、运行状态与证据"]
        FAILED -. 记录终止结果 .-> L1
        WU -. 记录暂停结果 .-> L1
        L1 --> L2["脱敏并生成唯一 event_id<br/>每个 state_id 最多一条事件"]
        L2 --> L3["追加 learning/ledger.jsonl"]
        L3 --> L4{"事件数量达到 compact_every"}
        L4 -- "否" --> END["学习流程完成"]
        L4 -- "是" --> L5["原始事件写入按内容寻址的 JSONL 归档"]
        L5 --> L6["确定性合并重复规则<br/>限制 active-rules 数量"]
        L6 --> END
    end
```
...
1. **启动控制**：工作流结构、`configured` 状态、核心锁和入口数据全部通过后创建运行状态。
2. **调度控制**：Runner 根据当前节点固定执行器、确认要求、超时和允许动作。
3. **验证控制**：节点 Schema、节点 validator、最终 Schema 和最终 validator 依次执行。
4. **失败控制**：重试次数、fallback、用户暂停和失败终止均由工作流与状态机决定。
5. **学习控制**：学习事件绑定状态 ID，经过证据校验和脱敏，达到阈值后先归档再压缩。
